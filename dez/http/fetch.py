...
def fetch(host, path="/", port=80, secure=False, headers={}, cb=None, timeout=5, json=False, dispatch=False, silent=SILENT, eb=None):
	http_client(silent).fetch(host, path, port, secure, headers, cb, timeout, json, eb)
	dispatch and do_dispatch()

def post(host, path="/", port=80, secure=False, headers={}, data=None, text=None, cb=None, timeout=5, json=False, dispatch=False, silent=SILENT, eb=None):
